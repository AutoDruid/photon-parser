...
func TestAsyncHookOnSessionCommandsNotAliasedWithPool(t *testing.T) {
	// Two commands: Acknowledge + VerifyConnect
	payload1 := []byte{
		0x0, 0x0, 0x0, 0x2, 0x16, 0x28, 0x80, 0x6b, 0x48, 0x74, 0x8c, 0x42,
		0x1, 0xff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x14, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x2e, 0x3, 0xff, 0x1, 0x0,
		0x0, 0x0, 0x0, 0x2c, 0x0, 0x0, 0x0, 0x0, 0xd7, 0xa1, 0x4, 0xb0,
		0x0, 0x0, 0x80, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x13, 0x88, 0x0, 0x0, 0x0, 0x2,
		0x0, 0x0, 0x0, 0x2,
	}
	// Two commands: Acknowledge + Ping (reuses same command pool on second parse)
	payload2 := []byte{
		0x79, 0x9e, 0x0, 0x2, 0x0, 0x0, 0x49, 0x1e, 0x7c, 0x11, 0x66, 0xbd,
		0x1, 0xff, 0x0, 0x4, 0x0, 0x0, 0x0, 0x14, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x8, 0xad, 0xb8, 0xd9, 0x60, 0x5, 0xff, 0x1, 0x4,
	...
	ch := parser.OnSessionAsync(types.HookOptions{Size: 2})

	var scratch photon.SessionV18

	if err := parser.ParsePacketInto(payload1, &scratch); err != nil {
		t.Fatalf("parse payload1: %v", err)
	}
	// Reuse pool before consumer reads async session from packet 1.
	if err := parser.ParsePacketInto(payload2, &scratch); err != nil {
		t.Fatalf("parse payload2: %v", err)
	}

	var got photon.SessionV18
	select {
	case got = <-ch:
	case <-time.After(time.Second):
		t.Fatal("timeout waiting for first async session")
	}

	// Header was copied by value — still correct.
	if got.Timestamp != 371753067 {
		t.Fatalf("timestamp: got %d, want 371753067", got.Timestamp)
	}
	if got.CommandCount != 2 {
		t.Fatalf("command count: got %d, want 2", got.CommandCount)
	}
	if len(got.Commands) != 2 {
		t.Fatalf("len(commands): got %d, want 2", len(got.Commands))
	}

	// Commands slice still aliases pool — second parse overwrote command data.
	if got.Commands[0].Type != types.AcknowledgeCommand {
		t.Fatalf("commands[0].type: got %v, want Acknowledge", got.Commands[0].Type)
	}
	if got.Commands[1].Type != types.VerifyConnectCommand {
		t.Fatalf("commands[1].type: got %v, want VerifyConnect (pool aliasing with payload2)", got.Commands[1].Type)
	}
}
